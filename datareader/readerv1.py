...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, h5_path, mode='omic', apply_sig=False,
                 shuffle=False, print_info=True, n_bins=4, ignore=[],
        ...
        slide_data = pd.read_hdf(h5_path, 'df')
    ...
    def return_all_splits(self, from_id: bool = True, csv_path: str = None):
        all_splits = pd.read_csv(csv_path)
        test_split = self.get_split_from_all_df(all_splits=all_splits, split_key='test')
        print("****** Normalizing Data ******")
        scalers = test_split.get_scaler()
        # train_split.apply_scaler(scalers=scalers)
        test_split.apply_scaler(scalers=scalers)
        # test_split.apply_scaler(scalers=scalers)
        ### <--
        # return train_split, val_split#, test_split
        return test_split, test_split

    def get_split_from_all_df(self, all_splits: dict, split_key: str = 'train', scaler=None):
        # print(all_splits)
        # print('-------------------------')
        # split1 = all_splits['train']
        # print(len(split1))
        # print('-------------------------')
        # split2 = all_splits['val']
        # print(len(split2))
        # print('-------------------------')
        # split = all_splits['train'] + all_splits['val']
        # print(split)
        # split = split.dropna().reset_index(drop=True)

        # train_splits = all_splits['train'].dropna().tolist()
        # val_splits = all_splits['val'].dropna().tolist()
        # split = train_splits + val_splits
        train_splits = all_splits['train'].dropna().reset_index(drop=True)
        val_splits = all_splits['val'].dropna().reset_index(drop=True)
        split = pd.concat([train_splits, val_splits]).reset_index(drop=True)
        print(split)
        print(len(split))
        if len(split) > 0:
            mask = self.slide_data['slide_id'].isin(split.tolist())
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx][:60660])
                    return (torch.zeros((1, 1)), genomic_features, label, event_time, c)
                    ...
                    global_wsi_path = data_dir.replace('WSI/pt_files', 'masks')
                    case_id = slide_id.replace('.pt', '.jpg')
                    global_path_img = Image.open(os.path.join(global_wsi_path, case_id)).convert("RGB")
                    preprocess = image_transform(
                        [448,448],
                        mean=(0.48145466, 0.4578275, 0.40821073),
                        std=(0.26862954, 0.26130258, 0.27577711),
                    )
                    global_img = preprocess(global_path_img)
                    omic_data = torch.tensor(self.genomic_features.iloc[idx].values)
                    gene_data = omic_data[:60660]
                    meth_data = omic_data[60660:]

                    # gene part:6, meth part:8, each one is nearly 10k
                    gene_size_each = 60660 // 6
                    meth_size_each = 80000 // 8
                    genes = [gene_data[i * gene_size_each:(i + 1) * gene_size_each] for i in range(6)]
                    gene1, gene2, gene3, gene4, gene5, gene6 = genes

                    meths = [meth_data[i * meth_size_each:(i + 1) * meth_size_each] for i in range(8)]
                    meth1, meth2, meth3, meth4, meth5, meth6, meth7, meth8 = meths

                    return (path_features, global_img,(gene1, gene2, gene3, gene4, gene5, gene6,
                                            meth1, meth2, meth3, meth4, meth5, meth6, meth7, meth8),
                            label, event_time, c)

                elif self.mode == 'coattn':  # TODO
                    path_features = []
                    for slide_id in slide_ids:
                        # wsi_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                        # wsi_bag = torch.load(wsi_path,map_location=torch.device('cpu'))
...
if __name__ == '__main__':
    dataset = Generic_MIL_Survival_Dataset(h5_path='/vip_media/SharedData/SurvivalPred/data/BRCA/Omic/omic_data.h5',
                                           data_dir='/vip_media/SharedData/SurvivalPred/data/BRCA/WSI/pt_files',
                                           mode='omic')
    train_dataset, val_dataset = dataset.return_splits(from_id=False,
                                                       csv_path='/vip_media/SharedData/SurvivalPred/data/BRCA/splits_0.csv')
