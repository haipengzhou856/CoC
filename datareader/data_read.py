...
LABEL_COL = 'survival_days' # the column name of the label where we used
N_BINS = 4 # discretization of the survival time, following previous work
SEED = 856 # my lucky number :)

class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path, mode='omic', apply_sig=False,
                  seed=SEED, print_info=True, n_bins=4,
...
class Gene_Dataset(Dataset):
    def __init__(self, gene_file):
        gene_data = pd.read_hdf(gene_file)
        gene_data = gene_data.drop_duplicates(['case_id']).copy()
    ...
    def __getitem__(self, idx):
        ...
        return gene_data, label, event_time,
