...
class CoC(nn.Module):
    def __init__(self, gene_sizes=[10110, 10110, 10110, 10110, 10110, 10110],
        ...
        # TODO
        self.gene_embed_dim = None
        self.meth_embed_dim = None
        self.wsi_embed_dim = None
        self.size_dict = {
            "path": [1024, 512, 256],
            "gene": [1024, 1024, 1024, 256],
            "meth": [1024, 1024, 1024, 256],
        }
        self.latent_dim = 256
        #########################################################################
        ######################### Raw Feature  Network  #########################
        ...
        # TODO: CoC Traction Network

        # Network
        ## 1-D data
        self.EncoderGene = nn.Sequential(
            *[nn.Linear(self.latent_dim * 6, self.latent_dim),
              nn.ReLU()]
        )
        self.EncoderMeth = nn.Sequential(
            *[nn.Linear(self.latent_dim * 8, self.latent_dim),
              nn.ReLU()]
        )

        ## 2-D data
        self.EncoderWSI = Transformer_P(self.latent_dim)
    ...
    def forward(self, **kwargs):
        ...
        feature_gene = torch.stack(feature_gene).unsqueeze(0)  # [1, 6, 256]
        feature_gene = feature_gene.flatten(1)

        feature_meth = [self.snn_meth[idx].forward(x)
                        for idx, x in enumerate(x_meth)]
        feature_meth = torch.stack(feature_meth).unsqueeze(0)  # [1, 8, 256]
        feature_meth = feature_meth.flatten(1)

        feature_wsi = self.fc_wsi(x_path).unsqueeze(0)  # [1, instance_num, 256]

        ########## finished, but they are heterogeneous ########
        cls_tk, _ = self.EncoderWSI(feature_wsi)
        fusion = self.fuse(torch.cat([cls_tk, self.EncoderGene(feature_gene), self.EncoderMeth(feature_meth)],
                                     dim=1))
        # TODO: fusion is the traction of all the modal data
        logits = self.classifier(fusion)  # [1, n_classes]
        hazards = torch.sigmoid(logits)
        S = torch.cumprod(1 - hazards, dim=1)
        return hazards, S,  # todo: return heatmap
...
class CoC_V2(nn.Module):
    def __init__(self, gene_sizes=[10110, 10110, 10110, 10110, 10110, 10110],
        ...
        self.size_dict = {
            "path": [1024, 512, 512],
            "gene": [1024, 1024, 1024, 512],
            "meth": [1024, 1024, 1024, 512],
        }
        self.latent_dim = 512
        ...
        self.intra_head = nn.Sequential(
            *[nn.Linear(self.latent_dim * 15, self.latent_dim),
              nn.ReLU(),
              nn.Linear(self.latent_dim, self.latent_dim),
              nn.ReLU()]
        )
        ...
        '''self.MAR = AutoregressiveDiffusion(
            dim_input=512,
            dim=512,
            max_seq_len=self.num_proj_token,
            depth=2,
            mlp_depth=3,
            mlp_width=512
        )'''
    ...
    def forward(self, **kwargs):
        ...
        ## CoC-1: visual-LocalGlobal prompt, in a hand-crafted way
        CoC_global_prompt = [
        ...
        ## MAR in training
        sos_target = torch.randn(1, 1, self.latent_dim).to(device)  #
        joint_target = torch.cat([sos_target, GlobalToken, LocalWSIToken], dim=1)
        pred, aux_loss = self.MAR(GlobalToken, LocalWSIToken, joint_target)
        homo_tokens = pred[:, 1:]
        # TODO
        ## 3. CoC-2: Omics prompt: ["an H&E stained image of {cancer}"]
        init_prompt = ["an H&E stained image of {cancer}"]
        gene_prompt = ['gene ']
        init_prompt = tokenize(texts=init_prompt, tokenizer=self.Tokenizer).to(device)
        with torch.no_grad():
            init_prompt = self.clip.encode_text(init_prompt).unsqueeze(0)
        gene_prompt = torch.cat((init_prompt, feature_gene), dim=1)  # [1,1+6,512]
        meth_prompt = torch.cat((init_prompt, feature_meth), dim=1)  # [1,1+8,512]
        ...
        # logits_gene_wsi = mix_Local_Global @ embed_gene.T
        # logits_meth_wsi = mix_Local_Global @ embed_meth.T

        inter_fea = self.inter_head(
            torch.cat([LocalWSIToken, feature_wsi_g, feature_gene, feature_meth], dim=1).flatten(1))
        intra_fea = self.intra_head(torch.cat([homo_tokens, embed_gene, embed_meth], dim=1).flatten(1))

        # TODO
        # deocder, given the preidciotn
    ...
    '''def forward_inference(self, **kwargs):
        ...
        inter_fea = self.inter_head(torch.cat([LocalWSIToken, feature_wsi_g], dim=1).flatten(1))
        intra_fea = self.intra_head(homo_tokens.flatten(1))
